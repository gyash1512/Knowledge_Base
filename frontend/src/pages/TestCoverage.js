...
const TestCoverage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await fetch('/api/test-coverage/gaps', {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-4xl font-bold text-white">Test Coverage Gap Detection</h1>
      <div className="bg-gray-800 p-8 rounded-lg shadow-lg">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="p-4 bg-gray-900 rounded-lg">
            <label htmlFor="code" className="block text-lg font-medium text-gray-300">Code</label>
            <textarea
              id="code"
              name="code"
              rows="10"
      ...
      {gaps && (
        <div className="mt-8 bg-gray-800 p-8 rounded-lg shadow-lg">
          <h2 className="text-2xl font-bold text-white mb-4">Gaps</h2>
          <div className="prose prose-invert">
            <ReactMarkdown>{gaps}</ReactMarkdown>
          </div>
        </div>
      )}
    </div>
